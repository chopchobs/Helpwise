...
✅ **§3.5 ตัดสินแล้ว 2026-08-09** — gate เดิมของ §F ปิดแล้ว (การตีความ (ข))

🔴 **แต่ยัง merge ไม่ได้ — §G ข้อ 13 (ใหม่ 2026-08-09)**
`MECHANISMS["readiness-probe"] = 300` × tolerance 3 = **900s** แต่ cron เต้นทุก **900s** ⇒ **margin = 0**
⇒ cron มาสาย = `stale` = `FAIL` · มาตรงเวลา = `OK` ⇒ **flap ทุก ~15 นาที ไม่รู้จบ**
⇒ เป็น alert fatigue ที่ §F กลัว **แต่มาจากประตูอีกบาน** และ **เกิดแน่นอนทันทีที่ merge**
⇒ ทางแก้ที่เสนอ (ยังไม่แก้ รอ Dev) → erratum **§H-11**

### ✅ **C0 ตอบแล้ว 2026-08-09 — ไม่บล็อกอีกต่อไป**

| | คำตอบ | ผล |
| --- | --- | --- |
| **C0-a** `DATABASE_URL` / `DIRECT_URL` | อย่างละ **1 แถว · "Production and Preview"** | **กรณี 1** — C1 apply/verify ครั้งเดียว · **Preview ใช้ DB ตัวเดียวกับ prod (แน่นอนแล้ว)** ⇒ §G ข้อ 9 มีผลจริง |
| **C0-b** `QSTASH_URL` | **1 แถว · "Production and Preview"** | **กรณี ข** — ⛔ **Delete ตายตัว** ⇒ D1 มีทาง A/B แล้ว |

🔴 **ข้อจำกัดใหม่ที่พบพร้อมกัน:** ทั้งสามตัวติดป้าย **`Sensitive`** ⇒ **อ่านค่ากลับไม่ได้เลย**
...
- [x] ~~**C0-a / C0-b**~~ → ✅ **ตอบแล้ว 2026-08-09** (กรณี 1 + กรณี ข · ดูตารางด้านบน)
- [ ] 🔴 **A-2 ของ D1 ทาง A** — พิสูจน์ว่า Vercel ยอมให้ key ซ้ำแบบ branch-scoped และ **precedence ชนะ**
      แถว "Production and Preview" · ⛔ **ห้ามเดาจากเอกสาร ต้องวัดผล** (หลักการเดียวกับ §G ข้อ 5)
      · ล้มเหลวได้อย่างปลอดภัย: precedence ไม่ทำงาน ⇒ ผลซ้อมเป็น `INVALID` ไม่ใช่ `PROVEN`

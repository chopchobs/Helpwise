...
### ⚖️ ประเมินความเร่งด่วนให้ตรงข้อเท็จจริง
**ความเสียหายต่อผู้ใช้จริง ณ 2026-08-05 = 0** — § 2-5 พิสูจน์แล้วว่า prod **ไม่มี tenant ลูกค้าจริงเลย**
มีแค่ `acme`/`globex` ที่เป็น demo → ประโยคที่เคยเขียนว่า *"กำลังทำร้าย user อยู่จริง (ลูกค้าไม่ได้รับเมล + SLA ไม่ทำงาน)"*
**ผิดข้อเท็จจริง** และเกณฑ์ "hotfix เพราะ user เจ็บอยู่" (ที่ใช้ตัดสิน `/portal` 404) **ใช้กับเคสนี้ตรง ๆ ไม่ได้**

**ยังควรแก้เร็ว แต่ด้วยเหตุผลที่ถูก:** demo คือ **portfolio ที่ visitor เปิดดูจริง** — feature ที่ตายเงียบในนั้น
= ความเสียหายเชิงธุรกิจ/ความน่าเชื่อถือ · และ **หนี้ยิ่งนานยิ่งแพง** (ของค้าง `PENDING` สะสมโดยไม่มี sweep)
เกณฑ์ที่ควรใช้ตัดสินคือ **"ต้นทุนของการปล่อยไว้"** ไม่ใช่ **"มีคนเจ็บอยู่ตอนนี้"**
...
### 🌅 งานแรกของพรุ่งนี้ (2026-08-06) — ตามลำดับ
1. **Upstash Console → QStash → Logs/Events** ช่วงเวลา smoke (2026-08-05 ~16:03 UTC)
   · **ไม่มี event เลย** = ยืนยันถูกปฏิเสธที่ producer (ตรงกับ log ที่เจอ) · **มี event + 401** = คนละสาเหตุ (ฝั่ง verify)
2. **QStash → Schedules** — มี schedule ชี้ `/api/jobs/sla-sweep` อยู่จริงไหม (ถ้าไม่มี = ยืนยัน SLA sweep ตายด้วย)
3. **เก็บกวาดของค้างจาก smoke** (ต้องใช้ session ของ Dev เอง): endpoint `cmsg9y3yr000004jltuk8blp1` บน `acme`
   + ปิด/ลบ bin ที่ webhook.site — ⚠️ **ความเสี่ยงเป็นศูนย์** (ยิงไม่ออกอยู่แล้ว) แต่ต้องเก็บให้จบตาม runbook § 4-2 ข้อ 4
4. ตัดสินใจตาม § ตัวเลือกด้านล่าง

### 🔀 ตัวเลือกที่ต้องตัดสิน: hotfix แยก vs รวม Phase 39
**สิ่งที่ต้องทำเพื่อแก้ region mismatch (ประเมินแล้ว ยังไม่ลงมือ):**
- `src/lib/queue.ts:242` สร้าง client ด้วย `new Client({ token })` **ไม่ได้ตั้ง baseUrl** → SDK ใช้ endpoint default
  ⇒ ถ้าบัญชี/token ผูกกับคนละ region จะถูกปฏิเสธตั้งแต่ publish (อาการที่เจอ)
- **ทางที่ไม่ต้องแตะโค้ด (ลองก่อน):** ออก `QSTASH_TOKEN` ใหม่จากบัญชี Upstash ปัจจุบัน แล้วตั้งบน Vercel Production
  ⚠️ **ต้องเปลี่ยน `QSTASH_CURRENT_SIGNING_KEY` + `QSTASH_NEXT_SIGNING_KEY` ให้เป็นชุดเดียวกับ token นั้นด้วย**
  (ถ้าเปลี่ยนแค่ token จะขยับจาก "publish ไม่ออก" ไปเป็น "worker 401 signature fail" = ย้ายปัญหา ไม่ได้แก้)
  · ตรวจ `QSTASH_TARGET_BASE_URL` = URL จริง (`https://acme.gethelpwise.xyz`) ไม่ใช่ `{slug}` template (`docs/deploy-checklist.md:86`)
- **ต้อง redeploy หรือไม่:** env เหล่านี้อ่านตอน runtime (ไม่ใช่ `NEXT_PUBLIC_*`) แต่ **Vercel ผูก env snapshot กับ deployment**
  → หลังแก้ค่า **ต้อง redeploy** จึงจะมีผลกับ deployment ที่ live
- **verify ครบ 3 feature (ห้ามเช็คแค่ webhooks):** (ก) webhooks → replay delivery ที่ค้าง แล้วต้องได้ `SUCCEEDED`
  (ข) outbound email → ส่ง agent reply แล้ว `TicketMessage.emailSentAt` ต้องไม่เป็น `null`
  ⚠️ **ข้อนี้อาจยังไม่ผ่านแม้แก้ QStash สำเร็จ** เพราะ `EMAIL_PROVIDER` อาจยังไม่ถูกตั้ง (ชั้นซ้อน)
  (ค) SLA sweep → ต้องเห็น invocation ของ `/api/jobs/sla-sweep` ใน Vercel log ตามรอบ schedule

| ตัวเลือก | ข้อดี | ข้อเสีย |
|---|---|---|
| **A · hotfix แยกก่อน** (แนะนำ) | ปลดล็อก 3 feature ที่ตายอยู่ทันที · ปิด gate ของ Phase 36 ได้ · เป็นงาน ops ไม่ต้องเขียนโค้ด | ยังไม่มีอะไรกันไม่ให้เกิดซ้ำ (P2 ยังไม่มา) |
| **B · รวมเข้า Phase 39** | แก้พร้อมสร้าง P2 ที่จะจับเคสนี้ได้ในอนาคต จบในรอบเดียว | 3 feature ตายต่อไปจนกว่า Phase 39 เสร็จ · ผูกงาน ops เข้ากับงานออกแบบ = ช้ากว่าที่ควร |

> **คำแนะนำ: A** — เกณฑ์เดียวกับ `/portal` 404 (แก้ของที่พังอยู่ ไม่ผูกกับกลไกป้องกันที่ต้องออกแบบ)
> **แต่คนละเหตุผล:** ครั้งนั้นเพราะ user เจ็บอยู่จริง · ครั้งนี้เพราะ **ต้นทุนการปล่อยไว้สูงและงานแก้เป็น ops ล้วน**
> (ดู § ประเมินความเร่งด่วน — prod ไม่มีลูกค้าจริง ความเสียหายต่อ user = 0)
> · ถ้าเลือก A: hotfix นี้จะกลายเป็น **หลักฐานทดสอบชิ้นแรกของ P2** ("ถ้ามี P2 ตอนนั้น มันจะจับเคสนี้ได้ไหม")
...
- **Phase 39: P2 + P3** → branch `feature/phase-39-server-env-readiness` จาก main
  - **P2** = `GET /api/health/readiness` สำหรับ **server-side resource เท่านั้น** (⛔ ห้ามมี key ของ `NEXT_PUBLIC_*`)
    ...
    🔴 **บทเรียนจาก QStash 2026-08-05 ที่ต้องออกแบบเข้าไปตั้งแต่ต้น:** *"เช็คว่า env มีค่า" จะให้ **PASS**
    ในเคสนี้ทั้งที่ระบบตายสนิท* (env มีค่าครบ แต่ credential ผิด region) → **P2 ต้องเรียก provider จริง**
    (เช่น QStash: ping/list schedule · email: ตรวจ provider จริงไม่ใช่ค่า `console`) ไม่ใช่ตรวจการมีอยู่ของ env

...
> เพิ่ม 2026-07-23 จาก security gate (verdict PASS-WITH-CONDITIONS, ไม่มี Critical/High) — Dev สั่งให้ LOW ทั้ง 2 + doc bug เข้า backlog, ปิดเฉพาะ MEDIUM-1/MEDIUM-2 ก่อน merge
- **[LOW security รอบ verify] fan-out `take` ไม่มี `orderBy`** — `src/lib/webhook-dispatch.ts:105-109` ถ้า enabled+subscribed > 10 (ข้อมูลเก่า/race) Postgres เลือก 10 แถวแบบไม่การันตีลำดับ → endpoint บางตัวขาด event **เงียบ ๆ สลับไปมาแต่ละ event**. แก้: `orderBy: { createdAt: "asc" }` + log เมื่อ `endpoints.length === MAX` (เจ้าภาพ `backend`, แก้ 1-2 บรรทัด)
- **[LOW security รอบ verify] route ที่ผลิต event ยังไม่มี rate-limit** — `src/app/api/portal/tickets/route.ts` + `src/app/api/tickets/route.ts` ไม่มี `checkRateLimit` (ยืนยันแล้ว: 0 จุด; `/api/v1/tickets` มี 3 จุด) → per-event bound แล้วที่ 10 แต่ **events/sec ยังไม่ bound** = amplifier 10x คงเหลือ. ความเสี่ยงต่ำ (ต้องมี session valid ใน tenant + ปลายทางเป็น URL ที่ admin ตั้งเองและผ่าน SSRF validation) และเป็นของเดิมก่อน Phase 36 → ไม่ใช่ merge blocker
- **[follow-up จาก MEDIUM-1] error code ใหม่ยังไม่อยู่ในเอกสาร + UI ไม่ handle** — `ENDPOINT_LIMIT_REACHED` (409) และ 429 ยังไม่มีในตาราง error code ของ `docs/webhooks.md` และ `docs/webhooks-contract.md` (§7) และยังไม่ระบุ cap 10 ที่ไหนเลย (เจ้าภาพ `docs-writer`) · `src/app/(agent)/(workspace)/settings/webhooks/page.tsx` ยังตกไป generic error handler ผู้ใช้ไม่รู้ว่าชน cap หรือโดน rate-limit (เจ้าภาพ `frontend`)
- **[LOW security] audit snapshot เก็บ webhook URL เต็ม** — URL อาจฝัง capability token (`https://hooks.slack.com/services/T00/B00/<token>`) หรือ userinfo `https://user:pass@host/hook` แล้วค้างถาวรใน `AuditLog` ที่ **immutable ลบไม่ได้**. จุดที่เก็บ: `src/app/api/webhook-endpoints/route.ts:206-210` (after.url) · `src/app/api/webhook-endpoints/[id]/route.ts:196-199` (before/after.url) · `:283-287` (before.url ตอนลบ). แนวทาง: เก็บแค่ `new URL(url).origin + pathname` แบบ redact + พิจารณา reject URL ที่มี userinfo ใน `validateWebhookUrl` (`src/lib/webhooks.ts:263-291`)
- **[LOW security] `WebhookDelivery.payload` เก็บเนื้อ message ของลูกค้าถาวร ไม่มี retention** — envelope ของ `ticket.message_created` มี `message.body` (`src/lib/webhooks.ts:435`) เขียนลงแถวที่ `src/lib/webhook-dispatch.ts:123`; grep ทั้ง repo ไม่มี job/cron ลบ delivery เก่าเลย → PDPA/GDPR data-minimization + ตารางโตไม่จำกัด. แนวทาง: cleanup job ลบ delivery `createdAt < now-30d` หรืออย่างน้อยล้าง `payload` ของ delivery ที่ SUCCEEDED เกิน N วัน + บันทึก retention ลง DPA (เจ้าภาพ `devops`/`database`)
- **[doc bug] `docs/webhooks.md` troubleshooting table เขียน `http_4xx`/`http_5xx` แต่ค่าจริงคือ `http_<status>`** — dispatcher เขียน `` errorMessage = `http_${responseStatus}` `` (`src/app/api/jobs/webhook-deliver/route.ts:194`) → ค่าที่เก็บจริงเป็น `http_502` / `http_301`; receiver ที่ grep หาสตริง `http_5xx` จะไม่เจอ. แก้ถ้อยคำเป็น `http_<status>` (มีในต้นฉบับไทยเดิมด้วย ไม่ใช่ของใหม่จากการแปล)

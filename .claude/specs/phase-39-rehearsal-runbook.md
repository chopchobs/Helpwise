...
> ## ✅ **ซ้อมครบ A→F แล้ว 2026-08-09** — E1 = `PROVEN` · F3 = `INVALID` (**ผ่านทั้งคู่**)
>
> ผลเต็ม + ตารางหลักฐาน → ขั้น **E** และขั้น **F** · สรุปรวม → `phase-39-closing-evidence-2026-08-09.md`
>
> **เหลือค้างของลำดับ 7 ข้อเดียว:**
> 🔴 **ตัว workflow ยังไม่เคยถูกรันเลย** — ซ้อมด้วย `npx tsx scripts/readiness-rehearsal.ts` จากเครื่อง
> (workflow ยังไม่ถูก register เพราะยังไม่ merge) ⇒ **พิสูจน์แล้วเฉพาะ *สคริปต์* ไม่ใช่ *workflow***
> ⇒ ดู **§2.9 ข้อ W** · erratum §G ข้อ 12 · เป็นขั้นบังคับ **หลัง** merge
>
...
## ✅ ผลจริง — **คืนสภาพสำเร็จ · 2026-08-09**

| ขั้น | ทำอะไร | หลักฐาน |
| --- | --- | --- |
| **F1** | ลบแถว `QSTASH_URL` scope **Preview + branch** (ที่สร้างใน D1 **ทาง A**) | dialog ยืนยัน *"Environments Using This Variable: **Preview**"* ⇒ **ไม่ใช่แถวของ Production** — verify-by-effect ก่อนกด ไม่ใช่หลังกด |
| **F2** | redeploy Preview | `dpl_4fzNqAeL…` · **READY** |
| **F3** | รันสคริปต์ซ้ำ · target `https://helpwise-llbhossbp-chopchats-projects.vercel.app` | ผลดิบด้านล่าง |

```
probe: http 503: FAIL
⇒ ได้ FAIL แต่ไม่พบ error signature ที่คาด
...
> 🔑 **`FAIL` ที่เหลืออยู่มาจาก `heartbeat_missing` (`sla-sweep`) — ไม่ใช่ร่องรอยของการซ้อม**
> `recordHeartbeat("sla-sweep")` ยังไม่อยู่บน `main` ⇒ ไม่มีใครเต้นให้ ⇒ `missing` ⇒ `FAIL`
> **เป็นพฤติกรรมที่ §F บังคับไว้เอง** ("ไม่พบ heartbeat = FAIL เสมอ") · ตรงกับที่ E2 เตือนไว้ล่วงหน้าแล้ว
> ⇒ **ถ้าตอนนี้ได้ `OK` ต่างหากที่ควรสงสัย**

> 🎁 **ผลพลอยได้: F3 เก็บกวาดสถานะที่ค้างจากการซ้อมไปในตัว**
> F3 เป็น **live probe** ⇒ `writeSnapshot()` **เขียนทับแถว `ReadinessState` singleton บน prod DB
> ด้วยผลหลัง*คืนสภาพ*แล้ว** ⇒ ผล `PROVEN` ของ E1 (ที่มี signature ติดอยู่) ไม่ค้างอยู่ในแถวที่
> endpoint สาธารณะจะเสิร์ฟ
>
> ⚠️ **ปิดเฉพาะผลข้างเคียง *ของรอบนี้* — ไม่ได้ปิดตัวข้อจำกัด §G ข้อ 9**
> `ReadinessState` ยังไม่มี scope ของ environment เหมือนเดิมทุกประการ · รอบหน้าถ้าซ้อมแล้วไม่ได้ทำ F3
> (หรือ F3 ชน min-interval แล้วได้ `source: stored`) สถานะของการซ้อมจะค้างอยู่บน prod เหมือนเดิม
> ⇒ **F3 ที่เป็น `live` จึงมีหน้าที่สองอย่าง: พิสูจน์การคืนสภาพ *และ* เก็บกวาดสถานะ**
> ⇒ นี่คือเหตุผลเพิ่มอีกข้อที่ **กฎที่ 0** (`source` ต้องเป็น `live`) สำคัญ — `stored` ไม่เก็บกวาดอะไรเลย

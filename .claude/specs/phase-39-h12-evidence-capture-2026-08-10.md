...
### 🔑 บันทึกการตัดสินเกณฑ์ — **Dev รับ H-1/H-2/H-3 เป็นเกณฑ์ที่ใช้จริง (2026-08-10)**

> ⚠️ **ตัดสิน *ก่อน* deploy และ *ก่อน* เห็นค่า `prev` — นี่คือเส้นแบ่ง**
> ถ้าตัดสินหลังเห็นผล จะเป็น**การขยับเป้า** · ตัดสินก่อน = **การแก้เกณฑ์ที่บกพร่อง**
> ⇒ บันทึกลำดับเวลาไว้เพราะสองอย่างนี้แยกกันไม่ออกในภายหลังถ้าไม่เขียนตอนนี้

**เกณฑ์เดิม** (`phase-39-closing-evidence-2026-08-09.md` ฉบับ 2026-08-09): *"เห็นข้อความ recovery จริง"*
**เหตุผลที่เปลี่ยน — สามข้อ ไม่ใช่ "เพราะ prod พัง":**

1. **แถว H วัด *"ความจำข้ามทริกเกอร์"*** ⇒ สิ่งที่ต้องวัดคือ **`prev` มีค่า (H-1) ตรงตัว**
   — คือหลักฐานว่า run บน event หนึ่ง **อ่าน state ที่ run บนอีก event เขียนไว้ได้**
2. **"ข้อความ recovery" เป็น *ผลข้างเคียง* ที่สังเกตได้เฉพาะตอน prod สุขภาพดี**
   และ *"ส่งข้อความได้จริง"* คือสิ่งที่ **แถว G** วัด — ซึ่ง **ปิดไปแล้ว 2026-08-10** (แถว 12)
   ⇒ เกณฑ์เดิมทำให้แถว H ไปวัดของที่แถวอื่นวัดไปแล้ว แทนที่จะวัดของตัวเอง
3. 🔑 **เกณฑ์ที่ผ่านได้ต่อเมื่อระบบสุขภาพดี ไม่ใช่เกณฑ์ของสิ่งที่มันอ้างว่าวัด**
   — ผู้เฝ้าถูกสร้างมาเพื่อทำงาน**ตอนระบบพัง** ⇒ เกณฑ์ที่ต้องรอให้ระบบหายก่อนถึงจะวัดได้
   **ขัดกับเหตุผลที่ผู้เฝ้ามีอยู่**

⛔ **เกณฑ์เดิมไม่ถูกลบ — เลื่อนเป็นหลักฐานเสริม:**
เมื่อ incident ถูกซ่อม transition `FAIL → OK` จะเกิดเอง ⇒ **ข้อความ recovery จะโผล่มาฟรี**
⇒ **เก็บเพิ่มตอนนั้น** ลง `$EVID` แล้วผนวกเข้าแถว H เป็นหลักฐานชิ้นที่ 4
⇒ ✅ **แต่มัน *เสริม* ไม่ใช่ *เงื่อนไขที่ต้องผ่านก่อนปิดแถว H*** — ห้ามกลับไปใช้เป็น gate อีก

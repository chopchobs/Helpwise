...
> ⚠️ **แก้เหตุผลที่ผิดข้อเท็จจริง (หลังรัน pre-check จริง 2026-08-05):** เหตุผลข้อหนึ่งที่เคยยกมาค้าน
> "เปิดที่ plan/global default" คือ *"จะกระทบ entitlement ของลูกค้าจริงทุกราย"* — **ข้อนี้ไม่จริง**
> § 2-5 พบว่า **prod มีแค่ `acme` + `globex` ไม่มี tenant ลูกค้าจริงเลยสักราย**
> **แต่ข้อสรุปยังเหมือนเดิม ด้วยเหตุผลที่แข็งกว่า:** ทาง A ตรวจ **ตรงชั้นที่ค่าถูกใช้จริง** ส่วนทาง B ไม่ —
> เหตุผลนี้ไม่ขึ้นกับว่ามีลูกค้าจริงกี่ราย
...
4. ⛔ **ลบ endpoint ที่สร้างตอน smoke — แต่ลบ "หลัง verify ผ่าน" เท่านั้น** (บังคับ — เหตุผลใน § 6)
   ...
   **เงื่อนไขการลบ (อย่าอ่านข้อนี้ว่า "ลบทันที" และอย่าอ่านว่า "ห้ามลบ"):**
   ...
   | สภาพตอนนี้ | ทำอะไร |
   | --- | --- |
   | ข้อ 3 ได้ `SUCCEEDED` แล้ว | ✅ ลบได้ — **แต่ต้องบันทึกหลักฐานลงเอกสารก่อนเสมอ ดูลำดับบังคับด้านล่าง** |
   ...
   **ใช้ไม่ได้เมื่อยังไม่มีข้อมูลถูกส่งออกไป**. ถ้า `attemptCount > 0` เมื่อไหร่ เหตุผลนั้นกลับมามีผลทันที
   ...
   ### 🔴 ลำดับบังคับ — บันทึกหลักฐานก่อน แล้วค่อยลบ (ห้ามสลับ)

   **1) บันทึกลงเอกสารก่อน** — อย่างน้อย 4 field ของทุก delivery ที่เกี่ยวข้อง:
   `status` · `attemptCount` · `lastAttemptAt` · `responseStatus`
   (⛔ **ห้ามบันทึก `responseBody`** — มี URL ของ bin ติดมาด้วย)

   **2) แล้วจึงลบ endpoint**

   > ⚠️ **`WebhookDelivery` ผูก `onDelete: Cascade` กับ `WebhookEndpoint`**
   > (`prisma/schema.prisma:796` — composite FK `[tenantId, endpointId]`)
   > ⇒ **ลบ endpoint = ลบหลักฐานทิ้งพร้อมกัน กู้ไม่ได้** · ยืนยันจริง 2026-08-06:
   > ลบ endpoint แล้ว `WebhookDelivery` เหลือ **0 แถว** ทันที
   > ⇒ ตัวเลขในเอกสารกลายเป็น **บันทึกเดียวที่เหลืออยู่** ของ smoke ทั้งรอบ
   >
   > 📌 รอบ 2026-08-06 **รอดเพราะบังเอิญ** มีการเขียน snapshot แทรกเข้ามาก่อน — ไม่ใช่เพราะ runbook สั่ง
   > (เดิม runbook ข้อนี้สั่งลบทันทีโดยไม่พูดถึง cascade เลย) จึงยกเป็นลำดับบังคับตั้งแต่บัดนี้

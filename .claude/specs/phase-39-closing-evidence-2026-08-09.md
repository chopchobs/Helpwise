...
**สถานะ:** 🔄 **ยังปิดไม่ได้** — มีแถวที่ยังว่าง (ดูตารางที่ 2) + **มีคำถามที่ Dev ต้องตัดสิน (§3.5)**
...
| F | **FeatureFlag** *(แถวบังคับตาม `CLAUDE.md` § Post-merge gate)* | — | **ยังไม่ประเมินว่าเกี่ยวกับเฟสนี้หรือไม่** — P2 ไม่มี feature gate (`hasFeature()`) ในเส้นทางใด ⇒ ถ้ายืนยันว่าไม่เกี่ยว ให้เขียนว่า **N/A พร้อมเหตุผล** ⛔ ห้ามลบแถวทิ้งเฉย ๆ | ⬜ |
...
> ⛔ **ผมไม่ตัดสินข้อนี้เอง** — เสนอการตีความพร้อมหลักฐาน ให้ Dev ตัดสิน
...
| --- | --- | --- |
| merge → prod deploy สำเร็จ | `deployment_status` (success + Production) ผ่านตัวกรอง ⇒ post-deploy poll ทำงาน · `sla-sweep` ยังไม่เต้น ⇒ `heartbeat_missing` ⇒ **`FAIL`** | ✅ **1 ข้อความ** (`shouldAlert(null, FAIL)` = true) |
| ≤ 5 นาทีถัดมา | QStash schedule ยิง `sla-sweep` บนโค้ดใหม่ ⇒ `recordHeartbeat("sla-sweep")` เต้น | — |
| รอบ cron ถัดไป (≤ 15 นาที) | live probe ⇒ heartbeat ครบ ⇒ **`OK`** ⇒ transition `FAIL → OK` | ✅ **1 ข้อความ** (recovery — **ตั้งใจให้แจ้ง**) |

⇒ **รวม 2 ข้อความ ภายใน ~20 นาที แล้วเงียบ**

> 🔑 **นี่ไม่ใช่ alert fatigue ตามนิยามของ §F** — สิ่งที่ §F กลัวคือ **`FAIL` ถาวร** (หอนไม่หยุดเพราะไม่มีตาราง)
> ของจริงคือ **transition สองครั้งที่จบในตัวเอง** ซึ่งเป็นพฤติกรรมที่ `shouldAlert()` ออกแบบมาให้เป็นแบบนั้น
> (transition-only + recovery ต้องแจ้ง)

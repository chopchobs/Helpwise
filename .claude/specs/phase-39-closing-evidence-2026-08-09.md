...
· 🔴 **§G ข้อ 15 + 16 (พบ 2026-08-09 จาก run จริงบน prod)** — ผู้เฝ้าจำข้ามทริกเกอร์ไม่ได้ ·
  และสเต็ปที่ไม่ทำงานขึ้นเขียว ⇒ ✅ **เลือกทางแก้แล้ว (ทาง A + assertion — §H-12)** ⛔ **ยังไม่ลงมือ**
  ⇒ **แถว H + I ยังว่าง**
...
| H | 🔴 **ความจำข้ามทริกเกอร์ของผู้เฝ้า** (§G ข้อ 15) | ✅ เลือก **ทาง A** แล้ว (§H-12) — พิสูจน์: deploy → ทำให้ `FAIL` → ปล่อยหาย → **ต้องได้ข้อความ recovery** | เห็นข้อความ recovery จริง | ⬜ **ยังไม่ลงมือ (รอรีวิวแบบ)** |
| I | 🔴 **assertion "สเต็ปเขียว ≠ สเต็ปทำงาน"** (§G ข้อ 16) | §H-12 ส่วนที่ 2 — สเต็ป verify ผ่าน **REST API** (`gh api .../actions/caches?key=…`) · ⛔ ไม่ใช้ `actions/cache/restore` (ผู้ตรวจต้องไม่ใช่ชิ้นส่วนเดียวกับผู้ถูกตรวจ) | job **แดง** เมื่อ save ไม่เกิดจริง (ทดสอบด้วยการทำให้ save ล้มโดยตั้งใจ) | ⬜ |

...
| H | 🔴 **ความจำข้ามทริกเกอร์ของผู้เฝ้า** (§G ข้อ 15) | ✅ **โค้ดเสร็จแล้ว** (ทาง A · §H-12) — อยู่บน `feature/phase-39-h12` **ยังไม่ merge** ⇒ **รอ `deployment_status` ตัวจริงเพื่อเก็บหลักฐาน** · ขั้นตอน + คำสั่ง + เส้นทางลบ → **`phase-39-h12-evidence-capture-2026-08-10.md`** | 🔴 **เกณฑ์ที่ใช้จริงอยู่ในเอกสารเก็บหลักฐาน (H-1/H-2/H-3)** — หลักฐานหลัก = log `mode=post-deploy … prev=` ที่ **`prev` ไม่ใช่ `—`**<br>⚠️ ถ้อยคำเดิม *"เห็นข้อความ recovery จริง"* เขียนไว้ **2026-08-09 ตอน prod ยัง `OK`** ⇒ **ใช้ไม่ได้ในสภาพปัจจุบัน** (prod `FAIL` ตั้งแต่ 02:12Z ⇒ ไม่มี recovery ให้เห็น) — **Dev ต้องรับเกณฑ์ใหม่ก่อนปิดแถว** | ⬜ |

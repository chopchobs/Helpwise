...
| 39 | **Server-env readiness (P2)** — readiness probe + heartbeat + inbound counter + GitHub Actions ผู้เฝ้า (transition-only) + ซ้อม reproduce incident Phase 38 ได้จริง · **P2 ทำงานบน prod แล้ว** (`verdict=OK`) | `feature/phase-39-server-env-readiness` | ✅ **ปิดเฟสแล้ว 2026-08-15 ที่ 18/21 แถว** (F = N/A) · เหลือ **A · C · I-b** ที่ **Dev ตัดสินว่าไม่ทำ** พร้อมเหตุผลรายแถว → `.claude/specs/phase-39-closing-evidence-2026-08-09.md` **§2.5** · เรื่องเล่าทั้งเฟส → `.claude/specs/phase-39-story-2026-08-15.md` · incident + ผลวัดซ้ำ **§11** → `.claude/specs/incident-2026-08-10-prod-503.md` | ✅ PR #17 (`585ae53`) — ✅ **3 commit เอกสารตามเข้า `main` แล้วที่ `b1b572b`** (verify 2026-08-10: `git diff main feature/phase-39-server-env-readiness` = ว่าง) |
| 40 | **Portal Contact UI** — PortalHeader (ชื่อผู้ใช้ + nav + ปุ่มออกจากระบบ) · 401 ทุกหน้า → `/portal/login` · `/portal` redirect · skeleton แทน spinner · responsive 375px · ยุบ DemoPersonaBanner เหลือแถวเดียว · **frontend-only** (ไม่แตะ backend/API/schema/proxy/lint gate) | `feature/portal-ui-polish` | ✅ done — ตรวจบนเบราว์เซอร์จริง 4 ข้อผ่าน (header หลัง verify ไม่ต้อง reload · skeleton shift 1.2px · `/portal` ตอนมี session · logout) · tsc clean · eslint 0 error · 1166 tests · known issues 5 ข้อบันทึกใน commit `617766d` | ✅ PR #26 (`f80a859`) |
...
1. 🔴 **แก้ 2026-08-10 — ข้อความเดิม *"ไม่มี Phase ค้าง"* เป็นเท็จมาตั้งแต่ Phase 36**
   ...
   · 🔴 **แต่ยังมี 3 เฟสที่ปิด gate ไม่ได้** — **36** (dispatch ตายบน prod) · **38** (ค้าง 2 อย่างที่ต้องใช้คนรันบน prod — ข้อ 11) · ~~**39**~~ ✅ **ปิดแล้ว 2026-08-15 ที่ 18/21** — A · C · I-b ประกาศเป็นของค้างที่รู้ตัวพร้อมเหตุผล (§2.5)

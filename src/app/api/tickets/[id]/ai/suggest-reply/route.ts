/**
 * POST /api/tickets/:id/ai/suggest-reply — ร่างคำตอบให้ลูกค้าด้วย Claude API (Phase 29 Slice 2)
 *
 * ⚠️ Tenant Isolation:
 ...
 *
 * ⚠️ Agent-Only: ผลลัพธ์ AI = internal draft — ห้ามหลุดฝั่ง portal
 *   - agent เห็นได้ทั้ง PUBLIC + INTERNAL message (นี่ฝั่ง agent ไม่ใช่ portal)
 *
 * ⚠️ DRAFT เท่านั้น (guardrail สำคัญสุดของ Slice นี้):
 *   - route นี้ "ห้าม" สร้าง TicketMessage / ส่ง email / mutate ticket ใด ๆ
 *   - คืนแค่ข้อความ draft ให้ frontend ใส่ใน reply box ให้ agent แก้ + กดส่งเองผ่าน flow เดิม
 *
...
import { suggestReply, AI_SUMMARY_MODEL } from "@/lib/ai";
import type { AiReplyDraftDTO } from "@/types/ai";
...
export async function POST(
  ...
  try {
    ...
    //    ⚠️ คืนแค่ draft string — ไม่สร้าง message / ไม่ส่ง email / ไม่ mutate
    let reply: string;
    try {
      reply = await suggestReply(messages);
    } catch (aiErr) {
      console.error(
        "[POST /api/tickets/:id/ai/suggest-reply] AI error:",
        aiErr instanceof Error ? aiErr.message : String(aiErr)
      );
      return NextResponse.json(
        { data: null, error: { code: "AI_ERROR", message: "ไม่สามารถร่างคำตอบได้ในขณะนี้" } },
        { status: 502 }
      );
    }

    // 7. Audit log — log แค่ metadata ห้าม log เนื้อหา/draft/PII
    void audit.log({
      tenantId,
    ...
    const data: AiReplyDraftDTO = { reply };
    return NextResponse.json({ data, error: null }, { status: 200 });
  } catch (err) {
    console.error(
      "[POST /api/tickets/:id/ai/suggest-reply] Unexpected error:",
